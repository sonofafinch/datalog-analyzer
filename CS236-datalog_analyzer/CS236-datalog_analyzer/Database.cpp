...
/*
	Adds variables to be used in projection, then projects current relation based
	on query variables found.
*/
void Database::searchProject(Predicate q_list, Relation& cur_rel)
{
	std::vector<std::string> vars; //Temporary vector to hold variables in query
	std::vector<Parameter> params = q_list.getParam(); //Get parameters for current query
	for (int i = 0; i < params.size(); ++i) //Iterate through parameters
	{
		if (!params[i].isLiteral()) //Variable?
		{
			vars.push_back(params[i].toString()); //Yes, add to vector
		}
	}
	cur_rel = cur_rel.project(vars); //Get relation with only those tuple positions listed in vector
}
